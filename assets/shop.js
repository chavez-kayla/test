...
jQuery( function ($) {
	...
	window.Site = {
		...
		sliders: {
			...
			setSlidesHeight: function(slider){
				...
				var currentSlideHeight = isSlideshow ? slider.find('.slick-current img:visible').height() : slider.find('.slick-current img').height();
			...
			initMobileSlider: function(currentSlider) {
				var isMobile = $(window).width() < 1920;
			...
			productTabs: function(){
				...
				$productTab.on('click', '.product-tab-title', function(){
					...
					//KJ
					/* if( $(this).parent('.product-tab').hasClass('is-active') ) {
						$(this).attr("aria-expanded", "true");
						$(this).next('.product-tab-content').attr("aria-hidden", "false");
					} else {
						$(this).attr("aria-expanded", "false");
						$(this).next('.product-tab-content').attr("aria-hidden", "true");
					} */
					//End KJ
				...
				//KJ
				$productTab.find('.product-tab-content').focus(function() {
					$(this).closest('.product-tab').toggleClass('is-active');
					
					if( $(this).closest('.product-tab').hasClass('is-active') ) {
						$(this).closest('.product-tab-content').attr("aria-hidden", "false");
					} else {
						$(this).closest('.product-tab-content').attr("aria-hidden", "true");
					}
		...
		nav: {
			...
			init: function(){
				...
				$navMobileClose = $('.nav-mobile-close');
				...
				$navMobile.on('keyup', function(e) {
					// Close if escape key pressed
					if (e.keyCode === 27) {
						Site.nav.hide();
					}
				});
				$navMobileClose.on('keyup', function(e) {
					// Close if enter key pressed
					if (e.keyCode === 13) {
						Site.nav.hide();
					}
					// Close if escape key pressed
			...
			bindings: function(){
				...
				//KJ
				$('.nav-standard .menu-item > a, .nav-standard .submenu .submenu-item--link, .nav-standard .submenu-image--link').focus(function() {
					if( $(this).hasClass('submenu-item--link') || $(this).hasClass('submenu-image--link') ) {
						if( $(this).closest('.megamenu-wrapper').css("opacity") == 0 ) {
							$(this).closest('.megamenu-wrapper').addClass('open');
							$(this).closest('.megamenu-wrapper').prev('a[aria-expanded]').attr('aria-expanded', true);
						}
					} else {
						$('.megamenu-wrapper.open').prev('a[aria-expanded]').attr('aria-expanded', false);
						$('.megamenu-wrapper.open').removeClass('open');
					}
				});
				//End KJ
			...
			show: function(){
				...
				$('.nav-mobile a[tabindex]').attr('tabindex', '2'); //KJ
				...
				$('body').on('keydown', function(e) {
					...
					if($(activeEl).hasClass('js-menuToggle') && e.which == 9) {
						$('.js-searchToggle').focus();
					} else if (e.which == 9) {
						var activeEl = document.activeElement;
			...
			submenu: {
				...
				close: function(el){
					...
					var $elSubMenu = el.closest('.submenu');
		...
		images: {
			loadBackgrounds: function() {
				...
				$elementsToLoad.each(function() {
					...
					if(src == '') {
						src = '//cdn.shopify.com/s/files/1/0031/2967/1749/t/22/assets/placeholder-pattern.png?v=183063600167011207251661791088';
						placeholder = true;
					}
					...
					$(im).on('error', function() {
						$el.css('background-image', 'url(//cdn.shopify.com/s/files/1/0031/2967/1749/t/22/assets/placeholder-pattern.png?v=183063600167011207251661791088)')
	...
	Cart = {
		...
		submit: function(el){
			if(el.hasClass('disabled') !== true) {
				...
				var $form = el.closest('form.productForm'),
					...
					variant_price = $('.product-price').attr('data-price'),
				...
				$.ajax({
					...
					success: function(product){
						...
						el.html("Added!");
						setTimeout(function(){
							el.html('<span class="product-price-only mobile-only">' + Shopify.formatMoney(variant_price).replace('.00', '') + ' - </span>' + "Add To Cart");
							}, 2000);

						
							Cart.getCart(Cart.updateCartCount);
					...
					error: function(data){
						$.ajax({
							...
							success: function(product){
								...
								setTimeout(function(){
									el.html('<span class="product-price-only mobile-only">' + Shopify.formatMoney(variant_price).replace('.00', '') + ' </span>' + "Add To Cart"); // swap it back
										}, 2000);
		...
		popover: {
			show: function(product){
				...
				item = {
					...
					price: product.price,
					price_formatted: Shopify.formatMoney(product.price).replace('.00', '')
				}
				
				//KJ
				if( product.variant_title && product.variant_title.includes("Custom Amount") ) {
					item["price_formatted"] = "";
				}
				//End KJ

				//KJ - Update variant metafields
		...
		isMixedCart: function(cart) {
			// KJ: check if Bold Subscription item is in cart
			var isMixedCart = false;
			$.each(cart.items, function(cartIndex, cartItem) {
				if( cartItem.properties.frequency_type_text || cartItem.properties.frequency_num) {
					isMixedCart = true;
				}
			});
			return isMixedCart;
		},
		showThresholdMessage: function(cart) {
			// KJ: show tiered discount threshold message on product page with updated cart total
			
			// Don't show discount threshold message if cart is mixed with Bold Subscription item(s)
			var isMixedCart = Cart.isMixedCart(cart);
			if(isMixedCart === true) return;
			
			var thresholdAmount1 = parseInt("0");
			var thresholdDiscount1 = "15% off";
			var thresholdAmount2 = parseInt("100");
			var thresholdDiscount2 = "20% off";
			var thresholdDiscount2Stacks = false;
			var thresholdAmount3 = parseInt("");
			var thresholdDiscount3 = "";
			var thresholdDiscount3Stacks = false;
			var cartTotal = (cart.original_total_price / 100).toFixed(2);
			var discountLevel = 0;
			if(thresholdAmount3 > 0 && cartTotal >= thresholdAmount3) {
				$('.cart_threshold_message .current_discount').html(thresholdDiscount3);
				$('.cart_threshold_message .next_discount').html("");
				discountLevel = 3;
			} else if (thresholdAmount2 > 0 && cartTotal >= thresholdAmount2) {
				$('.cart_threshold_message .current_discount').html(thresholdDiscount2);
				$('.cart_threshold_message .more .spend').html('$' + (thresholdAmount3 - cartTotal).toFixed(2));
				$('.cart_threshold_message .next_discount').html(thresholdDiscount3);
				discountLevel = 2;
			} else if (cartTotal >= thresholdAmount1) {
				$('.cart_threshold_message .current_discount').html(thresholdDiscount1);
				$('.cart_threshold_message .more .spend').html('$' + (thresholdAmount2 - cartTotal).toFixed(2));
				$('.cart_threshold_message .next_discount').html(thresholdDiscount2);
				discountLevel = 1;
			} else {
				$('.cart_threshold_message .message').html('');
				$('.cart_threshold_message .more .spend').html('$' + (thresholdAmount1 - cartTotal).toFixed(2));
				$('.cart_threshold_message .next_discount').html(thresholdDiscount1);
			}
			if( $('.cart_threshold_message .next_discount').html() == "" ) {
				$('.cart_threshold_message .more').html("");
			}
			if(thresholdDiscount2Stacks && discountLevel >= 2) {
				if(discountLevel == 2) {
					$('.cart_threshold_message .current_discount').html(thresholdDiscount1 + ' + ' + thresholdDiscount2);
				} else if(discountLevel == 3) {
					$('.cart_threshold_message .current_discount').html(thresholdDiscount3 + ' + ' + thresholdDiscount2);
				}
			}
			if(thresholdDiscount3Stacks && discountLevel >= 3) {
				if(discountLevel == 3) {
					$('.cart_threshold_message .current_discount').html(thresholdDiscount2 + ' + ' + thresholdDiscount3);
				}
			}
			$('.cart_threshold_message').show();
			//End KJ
		},
			...
			$.each(cart.items, function(index, cartItem) {
				...
				*/
				...
				item = {
					...
					price_formatted: Shopify.formatMoney(cartItem.price).replace('.00', ''),
			...
			$.getJSON(
				...
				function(product) {
					var id = product.id, // int
						...
						compare_at_price_formatted = Shopify.formatMoney(compare_at_price).replace('.00', ''),
						price_formatted = Shopify.formatMoney(price).replace('.00', ''); // string
					...
					if ( compare_at_price > price ) {
						...
						pricing += '<span><span class="clipped">Price Reduced From:&nbsp;</span><s class="product-compare-price">'+compare_at_price_formatted+'</s>&nbsp;</span>';
			...
			function callback(options){
				...
				if (variant) {
					if (variant.available) {
						$submit.removeClass('is-disabled').prop('disabled', false).css({'opacity': 1}).html('<span class="product-price-only mobile-only">' + Shopify.formatMoney(variant.price, moneyFormat).replace('.00', '') + '</span> - ' + "Add To Cart");
						$counter.css({'opacity': 1, 'pointer-events': 'auto'});
						$price.css({'opacity': 1});

						$price.attr('data-price', variant.price);
						$normal_price.html(Shopify.formatMoney(variant.price, moneyFormat).replace('.00', ''));

						if (variant.compare_at_price != null){
							if (variant.compare_at_price > variant.price) {
								if ($sale_container.length){
									$compare_price.html(Shopify.formatMoney(variant.compare_at_price, moneyFormat).replace('.00', ''));
									$sale_price.html(Shopify.formatMoney(variant.price, moneyFormat).replace('.00', ''));
								} else {
									$price.append('<div class="sale" itemprop="price"><span><span class="clipped">Price Reduced From:&nbsp;</span><s class="product-compare-price"></s>&nbsp;</span><span class="product-sale-price"></span></div>');
									$('.product-compare-price').html(Shopify.formatMoney(variant.compare_at_price, moneyFormat).replace('.00', ''));
									$('.product-sale-price').html(Shopify.formatMoney(variant.price, moneyFormat).replace('.00', ''));
								}
								$normal_price.hide();
								$sale_container.show();
								...
								if($normal_price.length) {
									...
									$price.append('<div class="product-normal-price" itemprop="price">'+Shopify.formatMoney(variant.price, moneyFormat).replace('.00', '')+'</div>');
								}
								$sale_container.hide();
								$normal_price.show();
							}
							
							//KJ - Update variant visibilities
							$('.variant-hide').hide();
							$('.variant-show-' + variant.id).show();
							$('.variant-show-' + variant.sku.replace(/\./g, '')).show();
							//End KJ
						} else {
							$sale_container.hide();
							$normal_price.show();
					...
					// this variant sold out
						...
						$submit.addClass('is-disabled').prop('disabled', true).css({'opacity': 0.3}).html('<span class="product-price-only"></span> - ' + "Add To Cart");
...
var Product = {
		...
		function manageOptions( obj ){
		  if (obj['options'].length === 1 && obj['variants'].length){
			if (obj['variants'][0].title === 'Default Title') {
			  ...
			  for (i = 0; i < obj['options'].length; i++) {
				$('#product-select-option-'+[i]).closest('.selector-wrapper').attr('data-id', 'product-select-option-'+[i]).prepend('<span class="selectArrow"></span><label for="product-select-option-'+[i]+'">'+obj['options'][0]+'</label>');
			  }
			}
		  } else if (obj['options'].length > 1){
			for (i = 0; i < obj['options'].length; i++) {
		...
		if (productImages > 1) {
			...
			$(window).on('scroll', function() {
				...
				if (windowWidth >= 1024) {
					...
					if ( scrolled >= productOffset && productInfoHeight < productImgScrollerHeight ) {
	...
	counter: function(){
		...
		$header.on('click', '.inputCounter-down', function(){
			var el = $(this);
			inputCounter(el, -1);
		}).on('click', '.inputCounter-up', function(){
			var el = $(this);
	...
	callback: function(options){
		...
		if($(options.selector.variantIdField).parents('.featured-product').length > 0){
				...
				$sale_container = $price.find('.sale'),
				$num_bubbles = $(fp).find('.bubbles-prompt .num-bubbles'),
			  $qty = $(fp).find('.quantity select[name="quantity"]'),
				...
				$sale_container = $price.find('.sale'),
				$num_bubbles = $('.bubbles-prompt .num-bubbles'),
			  $qty = $('.quantity select[name="quantity"]'),
		...
		if (variant) {
			...
			//KJ - Update variant dynamic properties
			$('.variant-remove-property').val("");
			$('.variant-add-property-' + variant.sku.replace(/\./g, '')).each(function() {
			...
			if (variant.available) {
				...
				$price.attr('data-price', variant.price);
				$normal_price.html(Shopify.formatMoney(variant.price, moneyFormat).replace('.00', ''));
				$num_bubbles.html( variant.price * 5 / 100 * $qty.val() );
			...
			// this variant sold out
				...
				$submit.addClass('is-disabled').addClass('sold-out').prop('disabled', true).css({'opacity': 0.3}).html("SOLD OUT");
				$shopbar_submit.addClass('is-disabled').html("SOLD OUT");
				$('.bubbles-prompt').addClass('remove-bubble-prompt');
				$counter.css({'opacity': 0.3, 'pointer-events': 'none'});
				$price.css({'opacity': 0.3});
		...
		if( $('.js-instafeed').length ) {
			$('.js-instafeed').each(function() {
				...
				var template = '<div class="instagram-img__wrapper %%featuredClass%%">\
									<a aria-label="%%caption%% - read more on Instagram" style="background-image:url(%%img%%);" class="instagram-img" target="_blank" href="%%link%%">\
	...
	request: function(query) {
		...
		self.searchRequest = $.ajax({
			...
			if (data.results_count) {
				...
				var results_to_subtract = 0;
				
				for ( var i = 0; i < data.results.length; i++ ) {
					var object_type = data.results[i].object_type;
					if (object_type == 'product')  {
						var product_price = data.results[i].price;
						if( product_price && product_price == 0) {
							results_to_subtract++;
						}
					}
				}
				results_count = results_count - results_to_subtract;
				//End KJ
				
				var resultsText = results_count == 1 ? results_count + " result" : results_count + " results";
...
/* Update num-bubbles when quantity selector is changed */
$(function() {
	$('.productForm .quantity select[name="quantity"]').on('change', function() {
		var variant_price = $('.product-price').data('price');
		$('.bubbles-prompt .num-bubbles').html( variant_price * 5 / 100 * $(this).val() );
	});
});
